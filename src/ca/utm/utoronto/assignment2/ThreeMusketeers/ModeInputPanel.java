...
public class ModeInputPanel extends GridPane {
    ...
    private int getFiles(ListView<String> listView) { // TODO
        ...
        if (files != null) { 
            for (File file : files) { 
              ...
              listView.getItems().add(i, file.getName());
              i++;
            } 
        } 
    	
        int starter = listView.getItems().indexOf("Starter.txt");
        
        return starter;
    }

    /**
     * Loads the board file selected in the boardsList and updates the selectBoardLabel with 
    ...
    private void selectBoard(Label selectBoardLabel, ListView<String> boardsList) { // TODO
    	...
    	String fileSelected = boardsList.getSelectionModel().getSelectedItem();
    	
    	this.view.model.setBoard(new File("boards/"+fileSelected));
    	selectBoardLabel.setText(fileSelected);
